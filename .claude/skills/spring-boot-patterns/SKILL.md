...
> **Rule: Use `@ConfigurationProperties`, NOT `@Value`**

```java
// ❌ BAD: @Value is brittle, not type-safe, scattered across classes
@Service
public class JwtService {
    @Value("${app.jwt.secret}")
    private String secret;           // No validation, no grouping

    @Value("${app.jwt.expiration}")
    private long expiration;         // Typo in key = silent null
}

// ✅ GOOD: @ConfigurationProperties — type-safe, validated, grouped, testable
...
// Inject the properties class
@Service
@RequiredArgsConstructor
public class JwtService {
    private final JwtProperties jwtProperties;  // Type-safe, IDE-navigable
}
```
...
|---|---|---|
| Type safety | No — string keys | Yes — bound to fields |
| Validation | Manual | `@Validated` + Bean Validation |
| IDE support | No navigation | Full autocomplete + navigation |
| Testability | Hard to mock | Inject as constructor arg |
| Grouping | Scattered across classes | One class per config group |
| Refactoring | Find-replace strings | Rename field, IDE handles it |
| SpEL expressions | Yes (rare need) | No (use `@Value` only for SpEL) |

**When `@Value` is acceptable:** Only for simple, one-off values that need SpEL expressions (e.g., `@Value("#{systemProperties['user.dir']}")`). For everything else, use `@ConfigurationProperties`.
